...
//        transform.rotation.z = 0;
        ...
        if (Axis == Axes.MouseXandY)
        {
            ...
            if(Input.mousePosition.x <= Screen.width / 2 - 100 || Input.mousePosition.x >= Screen.width / 2 + 100 ){
            	KeyLookAround();
            }
 
            // Call our look up and down function.
            if(Input.mousePosition.y <= Screen.height / 2 - 30 || Input.mousePosition.y >= Screen.height / 2 + 30){
            	KeyLookUp();
            }
        }
        ...
        {
            // Read the mouse input axis
            rotationX += (sensitivityX * Input.mousePosition.x);
    ...
    {
    	var distFromCenter = Input.mousePosition.x - (Screen.width / 2);
 
        // Call our Adjust to 360 degrees and clamp function
        Adjust360andClamp();
 
        // Transform our X angle
        cameraReference.transform.Rotate(Vector3.up * Time.deltaTime * distFromCenter);
        playerReference.transform.Rotate(Vector3.up * Time.deltaTime * distFromCenter * 1.06);
    }
 
    function KeyLookUp ()
    {
    	var distFromCenter = Input.mousePosition.y - (Screen.height / 2);
        // Adjust for 360 degrees and clamp
        Adjust360andClamp();
 
        // Transform our Y angle, multiply so we don't loose our X transform
        if((transform.rotation.y * 360) < maximumY && (transform.rotation.y * 360) > minimumY){ 
        	transform.Rotate(Vector3.left * distFromCenter * .05);
        }
//        playerReference.transform.Rotate(Vector3.right * Time.deltaTime * distFromCenter);
    }
